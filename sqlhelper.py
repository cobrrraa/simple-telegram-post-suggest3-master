...
from sqlalchemy import JSON  # Добавлено для использования типа JSON
...
class Post(Base):
    ...
    text = Column(String)
    messages = Column(JSON, default=[])  # Добавлено поле для хранения сообщений
    ...
    def __repr__(self):
        return '<Post(post_id={}, owner_id={}, attachment_filename={}, text={}>'.format(
            self.post_id,
            self.owner_id,
...
class Settings(Base):
    ...
    def __repr__(self):
        return '<Settings(initialized={}, target_channel={}, initializer_id={})>'.format(
            self.initialized,
            self.target_channel,
            self.initializer_id
        )
